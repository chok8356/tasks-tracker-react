...
A task tracking app for projects, backlog, board, issue details, team members,
and profile settings.
Stack: React 19, Vite, TypeScript, React Router, TanStack Query, Vitest,
Storybook.
...
Actions return `AsyncResult<T, InfraError>`:

- `ok: true` -> success result
- `ok: false` -> error
  - `infra` -> infrastructure error

The UI handles expected errors through `Result` instead of relying on
